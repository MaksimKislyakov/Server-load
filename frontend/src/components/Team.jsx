...
const Team = () => {
    ...
    useEffect(() => {
        if(localStorage.getItem('access_token') === null){                   
            ...
            (async () => {
                try {
                    const data = await axios.get(`https://irit-rtf-ep.ru/api/users/`, {
                        headers: {
            ...
            {users.map((user) => {
                return <div className="w-[1283px] h-fit bg-[#292C33] rounded-3xl p-6 flex justify-between items-center mb-6">
                    ...
                    <div className="flex flex-row items-start gap-6">
                        <img src={`https://irit-rtf-ep.ru${user.profile_photo}`} width='185' height='185' alt='Кнопка профиля' className="rounded-[50%]"/>
