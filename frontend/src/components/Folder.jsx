...
const Folder = () => {
    ...
    useEffect(() => {
        if(localStorage.getItem('access_token') === null){                   
            ...
            (async () => {
                try {
                    const data = await axios.get(`https://irit-rtf-ep.ru/api/event/${eventId}/`, {
                        headers: {
                    ...
                    const proj = await axios.get(`https://irit-rtf-ep.ru/projects/projects/${projId}/`, {
                        headers: {
                            'Content-Type': 'application/json',
                            'Authorization': `Bearer ${localStorage.getItem('access_token')}`
                        }
    ...
    function createFile(type, title, custom_name) {
        ...
        axios.post(`https://irit-rtf-ep.ru/projects/projects/${projId}/create_google_document/`, data, {
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('access_token')}`
            }
                ...
                <div className="flex items-center mb-[24px]">
                    ...
                    {/* <button className={`${buttonStyle} w-[200px]`} onClick={
                        (evt) => {
                            evt.preventDefault();
                            if (isEditing) {
                                axios.put(`https://irit-rtf-ep.ru/api/event/${eventId}/`, event, {
                                    headers: {
                                        'Authorization': `Bearer ${localStorage.getItem('access_token')}`
                                    }
                                })
