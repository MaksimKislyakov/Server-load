...
const Event = () => {
    ...
    useEffect(() => {
        if(localStorage.getItem('access_token') === null){                   
            ...
            (async () => {
                try {
                    const data = await axios.get(`https://irit-rtf-ep.ru/api/event/${eventData.id}/`, {
                        headers: {
                    ...
                    const usersData = await axios.get('https://irit-rtf-ep.ru/api/users/', {
                        headers: {
                            'Content-Type': 'application/json',
                            'Authorization': `Bearer ${localStorage.getItem('access_token')}`
                        }
                    ...
                    const proj = await axios.get(`https://irit-rtf-ep.ru/projects/projects/`, {
                        headers: {
                            'Content-Type': 'application/json',
                            'Authorization': `Bearer ${localStorage.getItem('access_token')}`
                        }
    ...
    const updateEvent = (updatedEvent) => {
        ...
        axios.put(`https://irit-rtf-ep.ru/api/event/${eventData.id}/`, updatedEvent, {
            headers: { 
                'Content-Type': 'application/json', 
                'Authorization': `Bearer ${localStorage.getItem('access_token')}` 
            }
    ...
    const handleDelete = (evt) => {
        axios.delete(`https://irit-rtf-ep.ru/api/event/${eventData.id}/`, {
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${localStorage.getItem('access_token')}`
            }
    ...
    function createFile(type, title, custom_name) {
        ...
        axios.post('https://irit-rtf-ep.ru/projects/projects/10/create_google_document/', data, {
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('access_token')}`
            }
    ...
    function createFolder() {
        ...
        axios.post('https://irit-rtf-ep.ru/projects/projects/create/', data, {
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('access_token')}`
            }
        } ,{ withCredentials: true })
                ...
                <div className="flex items-center mb-[24px]">
                    ...
                    <button className={`${buttonStyle} w-[200px]`} onClick={
                        (evt) => {
                            evt.preventDefault();
                            if (isEditing) {
                                axios.put(`https://irit-rtf-ep.ru/api/event/${eventData.id}/`, event, {
                                    headers: {
                                        'Authorization': `Bearer ${localStorage.getItem('access_token')}`
                                    }
                                })
